    ...
    <div class="card">
      ...
      <DataTable :value="questions" tableStyle="min-width: 50rem">
        ...
        <Column field="cardTypes" header="Cards Asked">
            <template #body="{ data }">
                <div v-for="card in data.cardTypes">
                    {{ cardStore.cardLabelLookup[card] }} 
                </div>
            </template>
        </Column>
        ...
        <Column field="cardTypeShown" header="Card Type Shown">
            <template #body="{ data }">
                {{ cardStore.cardLabelLookup[data.cardTypeShown] }} 
            </template>
        </Column>
        <Column field="actions" header="Actions">
          <template #body="{ data }">
            <AddEditQuestion title="Edit" @question-updated="editQuestion" :hands="hands" :question="data"/>
